...
import edu.cmu.qatar._15121.hw5.list.SimpleList;
...
public class Pellets {
	 ...
	 * @return `true` if a pellet lies on p; `false` otherwise
	 */
	 ...
	 *
	 * @return `true` if there are no more pellets; `false` otherwise
	 */
	...
	public void paint(Graphics g) {
		g.setColor(color);
		for (Point pellet : pellets) {
			g.fillOval(5 + SnakesGame.SEGMENT_SIZE * pellet.x + 2, 30 + SnakesGame.SEGMENT_SIZE * pellet.y + 2,
